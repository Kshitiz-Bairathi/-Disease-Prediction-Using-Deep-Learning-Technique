...
class CNNModel(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            ...
            nn.ReLU(),
            nn.BatchNorm2d(16),
            nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1),  # (16, 256, 256) -> (16, 256, 256)
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),  # (16, 128, 128)

            nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1),  # (16, 128, 128) -> (32, 128, 128)
            nn.ReLU(),
            nn.BatchNorm2d(32),
            nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1),  # (32, 128, 128) -> (32, 128, 128)
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),  # (32, 64, 64)

            nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1),  # (32, 64, 64) -> (64, 64, 64)
            nn.ReLU(),
            nn.BatchNorm2d(64),
            nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1),  # (64, 64, 64) -> (64, 64, 64)
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),  # (64, 32, 32)

            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # (64, 32, 32) -> (128, 32, 32)
            nn.ReLU(),
            nn.BatchNorm2d(128),
            nn.MaxPool2d(kernel_size=2, stride=2),  # (128, 16, 16)
            
            nn.Flatten(),  # (128 * 8 * 8) = 16384
            nn.Linear(128 * 16 * 16, 512),  # Added an intermediate fully connected layer
            nn.ReLU(),
            nn.Dropout(0.4),
