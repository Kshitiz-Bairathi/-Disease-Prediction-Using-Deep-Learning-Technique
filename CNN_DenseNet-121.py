...
class DenseNet121Custom(nn.Module):
    def __init__(self, fine_tune=True):
        ...
        # Optionally freeze all layers first
        for param in densenet121.parameters():
            param.requires_grad = False

        # Unfreeze last dense block (denseblock4) and transition3 for fine-tuning
        if fine_tune:
            for name, child in densenet121.features.named_children():
        ...
        densenet121.classifier = nn.Sequential(
            nn.Linear(1024, 512),  # DenseNet-121 output features are of size 1024
            nn.ReLU(),
            nn.Dropout(0.4),
...
for fold, (train_idx, val_idx) in enumerate(kf.split(combined_images, combined_labels)):
    ...
    model = DenseNet121Custom().to(device)
...
plt.legend()
plt.savefig("roc_curve_DenseNet121.png")