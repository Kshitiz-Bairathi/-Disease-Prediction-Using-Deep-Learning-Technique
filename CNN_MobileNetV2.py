...
class MobileNetV2Custom(nn.Module):
    def __init__(self, fine_tune_start_layer=14):
        ...
        # Unfreeze layers starting from a certain index in features
        for idx, layer in enumerate(mobilenetv2.features[fine_tune_start_layer:], start=fine_tune_start_layer):
            for param in layer.parameters():
                param.requires_grad = True

        # Replace the classifier with a custom head
        mobilenetv2.classifier[1] = nn.Sequential(
            nn.Linear(1280, 512),  # MobileNetV2 base outputs 1280-dim features
            nn.ReLU(),
            nn.Dropout(0.4),
